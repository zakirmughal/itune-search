...
const getAlbumArtists = async ({ params }) => {
    try {
        const { query, page } = params;
        if (!query) {
            throw new Error('Keyword is missing');
        }
        const { data, status } = await axios_1.default.get(`https://itunes.apple.com/search?term=${query}&media=music&attribute=albumTerm&limit=5&${page || 1}`);
        const response = [];
...
//# sourceMappingURL=list-all-albums.js.map